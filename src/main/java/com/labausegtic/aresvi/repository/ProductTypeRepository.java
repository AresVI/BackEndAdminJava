...
public interface ProductTypeRepository extends JpaRepository<ProductType, Long> {
    @Query("select distinct product_type from ProductType product_type left join fetch product_type.auditProcesses")
    List<ProductType> findAllWithEagerRelationships();

    @Query("select product_type from ProductType product_type left join fetch product_type.auditProcesses where product_type.id =:id")

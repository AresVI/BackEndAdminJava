...
@Component({
    selector: 'jhi-audit-process-recommendation',
    templateUrl: './audit-process-recommendation.component.html'
})
export class AuditProcessRecommendationComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.auditProcessRecommendations = [];
    ...
    trackId(index: number, item: AuditProcessRecommendation) {
        return item.id;
    }
    ...
    registerChangeInAuditProcessRecommendations() {
        this.eventSubscriber = this.eventManager.subscribe('auditProcessRecommendationListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.auditProcessRecommendations.push(data[i]);
        }
    }

...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/audit-process'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/audit-process', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: AuditProcess) {
        return item.id;
    }
    registerChangeInAuditProcesses() {
        this.eventSubscriber = this.eventManager.subscribe('auditProcessListModification', (response) => this.loadAll());
    }

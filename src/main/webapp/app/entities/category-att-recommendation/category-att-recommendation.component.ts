...
@Component({
    selector: 'jhi-category-att-recommendation',
    templateUrl: './category-att-recommendation.component.html'
})
export class CategoryAttRecommendationComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.categoryAttRecommendations = [];
    ...
    trackId(index: number, item: CategoryAttRecommendation) {
        return item.id;
    }
    ...
    registerChangeInCategoryAttRecommendations() {
        this.eventSubscriber = this.eventManager.subscribe('categoryAttRecommendationListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.categoryAttRecommendations.push(data[i]);
        }
    }

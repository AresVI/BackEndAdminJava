...
@Component({
    selector: 'jhi-category-attr-recommendation',
    templateUrl: './category-attr-recommendation.component.html'
})
export class CategoryAttrRecommendationComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.categoryAttrRecommendations = [];
    ...
    trackId(index: number, item: CategoryAttrRecommendation) {
        return item.id;
    }
    ...
    registerChangeInCategoryAttrRecommendations() {
        this.eventSubscriber = this.eventManager.subscribe('categoryAttrRecommendationListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.categoryAttrRecommendations.push(data[i]);
        }
    }

...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/weighting'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/weighting', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: Weighting) {
        return item.id;
    }
    registerChangeInWeightings() {
        this.eventSubscriber = this.eventManager.subscribe('weightingListModification', (response) => this.loadAll());
    }

...
@Component({
    ...
    templateUrl: './not_started.component.html',
})
...
export class NotStartedComponent implements OnInit {
    ...
    transition() {
        this.router.navigate(['/process/audit/status/not_started'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/process/audit/status/not_started', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
